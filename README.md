...
Development candidate 0.2.1. **Not submitted or represented as bounty-complete.** Public-library preview, single-reference import/repeat, manual-text preservation, and cursor citation/bibliography insertion have passed inside Amplenote. Automatic start/repeat/navigation/stop and a repeat after reload also pass. Full host coverage remains incomplete; see `HOST_TEST_RESULTS.md`.
...
The verified 0.2.1 build passes 70/70 tests. `evidence/verification.json` records the source/bundle hashes, runtime, test counts, and scope. Tests use synthetic Zotero responses, an in-memory Amplenote adapter, and a deterministic clock, including execution of the generated plugin object. The 25 automatic-sync cases cover opt-in, child updates, cancellation, overlap, settings changes, partial failures, and redacted status. Earlier host failures and their corrections are described in `HOST_TEST_RESULTS.md`.

`node live-smoke.mjs` can make a bounded request to Zotero's documented public example. Actual Amplenote observations are summarized in `HOST_TEST_RESULTS.md`; private authentication and file copying remain unverified.
...
`copyAttachments` enables copying uploaded Zotero PDF attachments to Amplenote. It defaults off. Linked local files are not copied; other attachments remain links to their source. Per-file limits default to 10 MiB and cannot exceed 50 MiB. The selected-item limit cannot exceed 10,000. Imports over 95,000 characters fail rather than silently truncate. These limits do not establish a maximum total account storage requirement.
